...
**新增地图先改 `maps/registry.js`，场景建模单独放在 `maps/<地图名>.js`。** 当前有酒馆（根目录 `tavern.js`）、训练场（`maps/training.js`）与神社（`maps/shrine-map.js`，细分模块见 `shrine/README.md`）、都市路口（`maps/urban-map.js`，细分见 `urban/README.md`）；没有麦田地图。
...
6. 当前战斗出生圈半径 6.8（combat.js:19），玩家摄像机远裁面默认 70（地图可用 far 元数据覆盖），血液边界仍沿用酒馆尺寸（fx.js），所以新增不同尺寸地图时要一起适配这三处；不要仅扩大场景模型。当前接口并不意味着任意大小地图已自动支持。
7. 增加地图后验证出生点、边界、家具挡拳、寻路、三档以上画质切换、反复切图资源释放，并同步主 README 的文件/函数/行号导航。

导航：registry.js:4 地图目录，:11 存储回退；manager.js:3 挂载/释放，picker.js:3 选图 UI；game.js:68 切图入口（以主 README 最新导航为准）。
...
训练场注册项使用 `mode: 'training'`：环境工厂仍只实现 arena 接口；人数、行为、补充规则位于根目录 `training.js`，暂停面板在 `training-panel.js`。game.js 按 mode 初始化，不把训练规则写入地图模型。

神社地图保留源项目整体环境，当前可战斗区域为中央半径 8.3 的平地石圈；周围神社台阶与建筑仅作为景观，不支持攀爬。`bright: true` 元数据启用明亮场景 HUD 对比度。

都市路口使用中央 ±11.4 的平地战斗区，边界扣除角色半径；路缘、人行道与外围街道为景观。注册项 far=210、exposure=1.16 保留原夜景，game.js:61 切图自动应用与恢复默认值。适配层不创建第二套 renderer、玩家控制或动画循环。
