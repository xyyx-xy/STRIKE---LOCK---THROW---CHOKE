...
export class CollisionWorld {
 ...
 ground(x,z,feet,step=.32){let h=0;for(const p of this.platforms)if(Math.abs(x-p.x)<=p.w&&Math.abs(z-p.z)<=p.d&&p.y<=feet+step)h=Math.max(h,p.y);return h;}
 resolve(pos,radius=.32,height=1.72){
  for(let pass=0;pass<4;pass++)for(const s of this.solids){if(pos.y>=s.maxY-.001||pos.y+height<=s.minY)continue;
   if(s.type==='circle'){let dx=pos.x-s.x,dz=pos.z-s.z,l=Math.hypot(dx,dz),need=s.r+radius;if(l<need){if(l<1e-8){dx=1;dz=0;l=1;}pos.x+=dx/l*(need-l);pos.z+=dz/l*(need-l);}}
