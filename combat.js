import {tickTraining} from './training.js';import {ARM_LOCK,startArmLock,tickArmLock} from './arm-lock.js';import {SNAP_KICK,snapImpact} from './snap-kick.js';import {guardBlocks} from './guard.js';
...
export function tick(s,p,dt,arena=null,rng=Math.random){const events=[];tickTraining(s,dt);if(s.ended)return events;s.time=(s.time||0)+dt;s.inv=Math.max(0,s.inv-dt);
 ...
 for(const e of s.enemies){if(e.hp<=0)continue;e.flash=Math.max(0,e.flash-dt);e.x+=e.vx*dt;e.z+=e.vz*dt;e.vx*=Math.exp(-5*dt);e.vz*=Math.exp(-5*dt);e.stun=Math.max(0,e.stun-dt);e.cool-=dt;const dx=p.x-e.x,dz=p.z-e.z,d=Math.hypot(dx,dz)||1,profile=mobility(e),arms=Number(!isGone(e,'Larm'))+Number(!isGone(e,'Rarm')),canAttack=arms>0&&!(e.kneel>0)&&profile.mode!=='stunned';e.guard=canAttack&&d<3&&e.wind<=0&&e.stun<=0&&profile.mode!=='crawl'&&profile.mode!=='drag'?(s.training&&s.training.guard!=='auto'?(s.training.guard==='none'?null:s.training.guard):(e.id%2?'torso':'head')):null;
