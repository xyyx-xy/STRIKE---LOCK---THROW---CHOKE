import {ARM_LOCK,startArmLock,tickArmLock} from './arm-lock.js';import {SNAP_KICK,snapImpact} from './snap-kick.js';import {guardBlocks} from './guard.js';
...
export function tick(s,p,dt,arena=null,rng=Math.random){const events=[];if(s.ended)return events;s.time=(s.time||0)+dt;s.inv=Math.max(0,s.inv-dt);
 ...
 if(s.attack?.type==='armLock')tickArmLock(s,dt,events);else if(s.attack){const a=s.attack,c=ATTACKS[a.type];a.t+=dt;if(!a.hit&&a.t>=c.active){a.hit=true;const contacts=s.enemies.filter(e=>e.hp>0&&targetInReach(p,e,c)&&(!arena||arena.clear(p,e))).map(e=>({e,h:specialStrike(p,e,c,a.type)})).filter(x=>x.h).sort((a,b)=>a.h.distance-b.h.distance);const contact=contacts[0];if(contact){const{e,h}=contact,result=damagePart(e,h.part,c.damage,rng);if(h.head&&c.headFatalChance&&e.hp>0&&rng()<c.headFatalChance){e.body.head.hp=0;e.hp=0;result.cause='head';result.fatal=true;}if(a.type==='uppercut'&&h.part==='torso'&&e.hp>0){e.slow=c.slowDuration;events.push({type:'slowed',enemy:e});}const knelt=a.type==='snapKick'&&snapImpact(e,h,result,rng);const d=Math.hypot(e.x-p.x,e.z-p.z)||1;e.vx=(e.x-p.x)/d*c.push;e.vz=(e.z-p.z)/d*c.push;e.stun=a.type==='kick'?.70:.4;e.wind=0;e.flash=.13;e.reaction={age:0,duration:a.type==='kick'?.95:.6,type:a.type,head:h.head,side:a.side?1:-1};events.push({type:'hit',enemy:e,attack:a.type,head:h.head,part:h.part,pos:h.pos,damage:result.damage,blocked:h.part.endsWith('arm')});if(knelt)events.push({type:'kneeling',enemy:e});if(result.severed)events.push({type:'sever',enemy:e,part:h.part,pos:h.pos});if(result.stunned)events.push({type:'stunned',enemy:e});if(result.cause)down(s,e,events,result.cause,a.type);}else events.push({type:'miss'});}if(a.t>=c.duration)s.attack=null;}
 for(const e of s.enemies){if(e.hp<=0)continue;e.flash=Math.max(0,e.flash-dt);e.x+=e.vx*dt;e.z+=e.vz*dt;e.vx*=Math.exp(-5*dt);e.vz*=Math.exp(-5*dt);e.stun=Math.max(0,e.stun-dt);e.cool-=dt;const dx=p.x-e.x,dz=p.z-e.z,d=Math.hypot(dx,dz)||1,profile=mobility(e),arms=Number(!isGone(e,'Larm'))+Number(!isGone(e,'Rarm')),canAttack=arms>0&&!(e.kneel>0)&&profile.mode!=='stunned';e.guard=canAttack&&d<3&&e.wind<=0&&e.stun<=0&&profile.mode!=='crawl'&&profile.mode!=='drag'?(e.id%2?'torso':'head'):null;
 if(e.ko>0||e.kneel>0){e.wind=0;e.atk=null;}else if(e.stun===0){if(e.atk){advanceEnemyAttack(s,p,e,dt,arena,profile,arms,canAttack,d,events);}else if(e.cool<=0&&canAttack){const pool=e.moves&&e.moves.length?e.moves:['palm'],avail=pool.filter(m=>d<ATTACKS[m].range*.95),m=avail.length?avail[Math.floor(rng()*avail.length)]:null,c=m&&ATTACKS[m];if(m&&(!arena||arena.clear(e,p))){e.attackCount=(e.attackCount||0)+1;const side=isGone(e,'Larm')?1:isGone(e,'Rarm')?0:e.id%2;const zone=(profile.mode==='crawl'||profile.mode==='drag')?'torso':(['punch','hook','uppercut'].includes(m))?'head':(e.attackCount+e.id)%2?'head':'torso';e.atk={type:m,side,t:0,hit:false,zone};e.wind=c.duration;e.attackSide=side;e.attackZone=zone;}else if(d>1.05){const speed=(1.15+Math.min(s.wave,8)*.15)*profile.speed*(e.slow>0?ATTACKS.uppercut.slowFactor:1),target=arena?arena.steer(e,p):p,tx=target.x-e.x,tz=target.z-e.z,td=Math.hypot(tx,tz)||1;e.x+=tx/td*speed*dt;e.z+=tz/td*speed*dt;}}else if(d>1.05){const speed=(1.15+Math.min(s.wave,8)*.15)*profile.speed*(e.slow>0?ATTACKS.uppercut.slowFactor:1),target=arena?arena.steer(e,p):p,tx=target.x-e.x,tz=target.z-e.z,td=Math.hypot(tx,tz)||1;e.x+=tx/td*speed*dt;e.z+=tz/td*speed*dt;}}
...
export function specialStrike(p,e,config,type) {
  const direct=strikeHit(p,e,config);
...
function advanceEnemyAttack(s,p,e,dt,arena,profile,arms,canAttack,d,events) {
  ...
  if(!a.hit&&a.t>=c.active) {
    ...
    if(canAttack&&d<c.range&&s.inv===0&&(!arena||arena.clear(e,p))) {
      const zone=a.zone,blocked=guardBlocks(s.guard,zone,p,e);
      if(!blocked) {
      ...
      s.inv=.6;
      events.push({type:blocked?'block':'hurt',zone,move:a.type,heavy:a.type==='kick'||a.type==='snapKick'});
    }
  }
  if(a.t>=c.duration) {
    e.atk=null;
