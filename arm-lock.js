...
export function startArmLock(s,e,hit,q,rng=Math.random){
  if(!extendedArm(e,hit.part,q)||rng()>=.5)return false;
...
export function lockHands(rig,a){
  ...
  const grip=smooth(a.t/.12)*(1-smooth((a.t-.56)/.29)),fold=smooth((a.t-.20)/.30)*(1-smooth((a.t-.56)/.29));
  const lead=a.side?'L':'R';
  for(const [side,h]of Object.entries(rig.hands)){
    for(const [f,f2]of h.fingers){f.rotation.x=.12+grip*.95;f2.rotation.x=.08+grip*1.12;}
    h.thumb.rotation.x+=(-.85-h.thumb.rotation.x)*grip;
    h.group.rotateZ((side==='L'?-1:1)*grip*(side===lead?.85:-.40));
    h.group.rotateX((side===lead?-.80:.25)*fold);
  }
}
