...
当前仅开放 **「打」· 酒馆打击试炼**：掌击、正蹬、走位、受击与击倒。「極」「投」「絞」尚未实现；受击倒地不等同于主动投技。
...
4. 源码在本目录；`~/Library/Caches/da-ji-tou-jiao-preview` 是启动器生成的预览副本。修改源码后同步预览，不要只改缓存。
5. 保留其他开发者已有修改，尤其 v0.1 正蹬分段、第一人称踢腿关键帧及掌形。涉及它们时同时核对下列多个入口。

导航核对日期：2026-09-20（v0.3.1）。新增代码请采用可读的分行格式。
...
- WASD 走位、Shift 快步、鼠标环视；方向键也可转向。左键交替掌击，右键交替正蹬。
- 按住 Q 护头、E 护躯干；匹配高度的正面攻击可格挡。P / ESC 暂停；锁定鼠标失败时使用直接环视。
- 暂停页编排六种单招，三套招式分别按 1 / 2 / 3 施放，X 取消剩余动作；每套最多 12 步，防守 0.2–3 秒。保存仅属于当前浏览器、当前地址。
...
| `game.js:10–12` | `renderer / camera / arena / handsScene` | 渲染器、主摄像机、酒馆入口、独立第一人称手脚场景 |
| `game.js:36` | `frame / resize` | 主场景和手脚的分层渲染、视角、震动、窗口尺寸 |
...
| 文件与行号 | 入口/部分 | 负责内容 |
|---|---|---|
| `rig.js:3` | `links` | 躯干、上臂、小臂、大腿、小腿的骨段连接、宽深尺寸和材质分类 |
| `rig.js:5` | `solveLeg` | 双段腿部 IK，膝盖朝前，避免后退时反关节 |
| `rig.js:6` | `pose` 基础姿态 | 身高、肩髋位置、站姿、前后左右步态、抬脚 |
| `rig.js:7` | `pose` 的 `attack` 分支 | 左右掌击、正蹬提膝/伸腿/收腿、躯干与手臂配合 |
| `rig.js:8` | `reactPose` | 命中后的扭身、仰头、弯腰、手臂摆动、受击撤步 |
| `rig.js:9–10` | `FighterRig.constructor` | 方块骨段和关节、皮肤/衣服/裤子材质、头手脚端点模型 |
| `rig.js:11` | 头部附属网格 | 眼睛、眉毛、鼻、嘴、头发 |
| `rig.js:12–13` | 手掌网格、`setHands` | 掌、手指两节、拇指构建和开合，保留用户掌形 |
| `rig.js:14–16` | `set / react / flash` | 关节点驱动网格位置旋转缩放、头部受击转动、起手/受击发光 |
| `rig.js:19–22` | `Ragdoll` | 倒地位置约束、重力、地面摩擦和边界；未做完整自碰撞及家具支撑 |
| `rig.js:25` | `injuryPose` 爬行分支 | 缺腿双臂爬行、单臂拖行、无臂挪行、击晕伏地 |
| `rig.js:26–27` | `injuryPose` 站立分支 | 单脚缺失跛行、调用 NPC 防守姿态 |
| `rig.js:28` | `enemyPose` | 组合基础动作、攻击高度、受击和伤势姿态 |
| `rig.js:29` | `FighterRig.prototype.setInjuries` | 断臂、断腿、断脚后隐藏对应网格、关节和手掌 |
| `guard.js:2–4` | `guardPose / guardBlocks / guardHands` | 合臂护头、三战护躯干的肘腕位置；玩家格挡方向/高度；张掌朝向 |
| `game.js:12–14` | `playerRig` 初始化 | 第一人称模型的摄像机挂载和位置 |
| `game.js:32` | NPC 骨架更新 | 世界坐标、朝向、步速、手型、防守、断肢显示、受击反馈 |
| `game.js:35` | `handPose`、`ka` 正蹬分支 | 第一人称手臂偏移、玩家踢腿关键帧；与 `rig.js:7` 的第三人称正蹬分别维护 |
...
| 文件与行号 | 入口/部分 | 负责内容 |
|---|---|---|
| `combat.js:4–7` | `ATTACKS / createState / beginAttack / targetInReach` | 攻击伤害、距离、时长、连击收招限制、初始状态 |
...
| `game.js:18–29` | 状态、`notify / reset / pause`、输入监听 | 暂停、重开、鼠标锁定、键盘和鼠标绑定；扣血提示由 `notify` 写入 |
| `game.js:30` | `effects` | `hit / hurt / block / sever / bleed / down / wave` 事件映射到文字、血液、音效、停顿和震屏 |
| `game.js:31–35` | `simulate` | 每个模拟步处理手动防守、连招、移动、战斗和模型姿态 |
| `game.js:36–39` | `frame / updateTarget`、部位行初始化 | 玩家生命、波次、击倒计数、瞄准部位、目标血条和失血数值 |
| `combos.js:1–5` | `MOVES / validateCombo / newPlayback / stepPlayback / loadCombos` | 六种单招目录、输入校验、按收招时序播放、读取三套招式 |
...
```sh
node --test combat.test.mjs animation.test.mjs injury.test.mjs guard-combo.test.mjs
```
