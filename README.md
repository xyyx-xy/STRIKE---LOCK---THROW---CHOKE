...
「打」=打击，「極」=关节技，「投」=摔投，「絞」=绞技。已实现打击与臂锁原型；主动摔投、绞技、武器尚未实现。当前只有酒馆地图。
...
导航核对：2026-09-21，v0.9.5。本次仅整理文档，不改变游戏行为。
...
| 环境与新地图 | `maps/README.md` → `maps/registry.js:3` | `tavern.js:3` 现有酒馆；`maps/picker.js:3` 选图 |
...
| `game.js:48,50,54,60–73` updateTarget / useBinding / updateBindingLabels / refreshQuality | 部位 HUD、单招输入、按键提示、画质与切图装配 |
...
| `combos.js:2–6` MOVES / validateCombo / newPlayback / stepPlayback / loadCombos | 15 个单招目录、连招校验/播放/存储；臂锁失败跳过该步 |
...
| `arm-lock.js:2,3,10,16` ARM_LOCK / extendedArm / startArmLock / tickArmLock | 伸直攻击臂检测、成功率、抓取与延迟断臂；空挥仅播放动画 |
| `arm-lock.js:32,41,54,66` LOCK_FRAMES / armLockPose / lockedEnemyPose / lockHands | 叠臂关键帧、玩家镜像动作、敌人受控姿态、双手抓握朝向 |
| `game.js:41` NPC 骨架更新 | 调用 showFists：拳法用拳、掌击/防守用掌；待机/踢腿按 moves 首个手部招式，没有则用掌 |
| `game.js:44` handPose / ka / lockHands | 玩家第一人称偏移与独立正蹬关键帧；拳掌切换、臂锁手型 |
| `viewmodel.js:2` updateViewVisibility | 第一人称手臂与出招腿的可见性 |
| `previews/arm-lock.html:1`、`previews/arm-lock.js:1–18` | 独立臂锁检视页：双视角、滑杆、播放、镜像；不接入战斗或存档 |
...
| `maps/registry.js:3,7` MAPS / loadMap | 地图工厂、名字、出生点、预览视角、卡片元数据与存储回退 |
...
地图目前仅酒馆。改画质不重开，切换不同地图才重开。新增地图先读 `maps/README.md`，特别核对出生圈、远裁面和血液边界；不要假定任意尺寸已受支持。
...
存储键统一前缀 `da-ji-tou-jiao.`：`bindings.v1`（四键）、`combos.v1`（连招）、`quality`（画质）、`map`（地图）。仅当前浏览器/地址有效；不是战斗进度存档。
...
| 摆勾拳、弹腿、臂锁 | `curved-punch.test.mjs`、`snap-kick.test.mjs`、`arm-lock.test.mjs` |
